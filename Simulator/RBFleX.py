...
class RBFleX:
    def __init__(self,
        ...
        if self.benchmark == "sss":
            ...
            print("___The total size of designspace___", len(self.searchspace))
            pass
            # sys.exit()
        ...
        # self.GAMMA_K = 1.2582787637712191e-08
        # self.GAMMA_Q = 1.1573391811901056e-06
        print('Hyperparameter\n\t[Activation: {}, Final input: {}]'.format(self.GAMMA_K, self.GAMMA_Q))

    def run(self, image, arch, hw):
        uid = 0
        ########################
        # NAS
        ########################
        if self.benchmark == "sss" or self.benchmark == "201":
            ...
            config = self.searchspace.get_net_config(uid, 'cifar100') # cifar10, cifar100, ImageNet16-120
        ...
        # print("==> Writing..")
        # output_dir = os.path.join('inputs/WL', hw, '/', str(uid))
        output_dir = 'inputs/WL/' + hw + '/' + str(uid)
        pass
        ...
        # sys.exit()
        return network_score * 10, backbone, self.layers_dist, uid
    ...
    def converted_layers_info(self, layers_dist):
        ...
        for idx, (layer_name, layer_info) in enumerate(layer_dist.items()):
            if layer_info['layer'] == 'convolution':
                ...
                # K is for output channel;
                # C is for input channel;
                # OX and OY are feature map spatial dimensions;
                # FX and FY are weight spatial dimensions.
                if self.hw == "Meta_prototype_DF" or self.hw == "Meta_prototype":
                    default_convolution_spatial_mapping = {'D1': ('K', 32), 'D2': ('C', 2), 'D3': ('OX', 4), 'D4': ('OY', 4)}
                elif self.hw == "TPU_like" or self.hw == "TPU_like_DF":
                    default_convolution_spatial_mapping = {'D1': ('K', 32), 'D2': ('C', 32)}
                elif self.hw == "Edge_TPU_like" or self.hw == "Edge_TPU_like_DF":
                    default_convolution_spatial_mapping = {'D1': ('K', 8), 'D2': ('C', 8), 'D3': ('OX', 4), 'D4': ('OY', 4)}
                elif self.hw == "Ascend_like" or self.hw == "Ascend_like_DF":
                    default_convolution_spatial_mapping = {'D1': ('K', 16), 'D2': ('C', 16), 'D3': ('OX', 2), 'D4': ('OY', 2)}
                elif self.hw == "Tesla_NPU_like" or self.hw == "Tesla_NPU_like_DF":
                    default_convolution_spatial_mapping = {'D1': ('K', 32), 'D3': ('OX', 8), 'D4': ('OY', 4)}
                else:
                    default_convolution_spatial_mapping = {'D1': ('K', 32), 'D2': ('C', 2), 'D3': ('OX', 4), 'D4': ('OY', 4)}
                # D1, D3, D4 stands for one of hardware resource dimensions
                # '32' stands for how many parallel processor are allocated to this dimension's computations
                # In here, the largest numbers of required PEs is 42
                converted_layer = {
                    ...
                    # Different mapping indicates how all dimensions of input feature map mapping to output feature map
                    # or the intermedia variables G (group)
                    'constant_operands': ['W'],
                    'core_allocation': 1,
                ...
                params = layer_info['param']
                default_batchnorm_spatial_mapping = {'D1': ('K', 32), 'D3': ('OX', 4), 'D4': ('OY', 4)}
                # default_batchnorm_memory_operand_links = {'O': 'O','I': 'I1','mean': 'mean','var': 'var','weight': 'weight','bias': 'bias'}
                # default_batchnorm_operand_precision = {'O': 16, 'O_final': 8, 'W': 8, 'I': 8, 'mean': 16, 'var': 16, 'weight': 16, 'bias': 16}
                default_batchnorm_memory_operand_links = {'O': 'O', 'I': 'I1'}
                default_batchnorm_operand_precision = {'O': 16, 'O_final': 8, 'I': 8}
                converted_layer = {
                    # 'equation': 'O[b][c][oy][ox]=(I[b][c][oy][ox]-mean[c])/sqrt(var[c])*weight[c]+bias[c]',
                    'equation': 'O[b][k][oy][ox]=I[b][k][oy][ox]',
                    'equation_relations': [],
                    'loop_dim_size': {'B': 1, 'K': params['out_channel'], 'OY': params['output_size'][2], 'OX': params['output_size'][3]},
                    'operand_precision': default_batchnorm_operand_precision,
                    'operand_source': {'I': [pre_idx]},
                    'operand_source_dimension_mapping': {'I': {'OX': 'OX', 'OY': 'OY', 'K': 'K'}},
                ...
                params = layer_info['param']
                default_relu_spatial_mapping = {'D1': ('K', 32), 'D2': ('C', 2), 'D3': ('OX', 4), 'D4': ('OY', 4)}
                default_relu_memory_operand_links = {'O': 'O', 'I': 'I1'}
                default_relu_operand_precision = {'O': 16, 'O_final': 8, 'I': 8}
                converted_layer = {
                    'equation': 'O[b][k][oy][ox]=I[b][k][oy][ox]',
                    'loop_dim_size': {
                        'B': 1,
                ...
                params = layer_info['param']
                padding = params['padding']  # not used yet
                default_spatial_maxpool_mapping = {'D1': ('G', 32), 'D3': ('OX', 4), 'D4': ('OY', 4)}
                converted_layer = {
                    'equation': 'O[b][g][oy][ox]+=W[fx][fy]*I[b][g][ix][iy]',
                    'equation_relations': [
                        f'ix={params["stride"]}*ox+1*fx',
                        f'iy={params["stride"]}*oy+1*fy'
                    ],
                    'loop_dim_size': {'B': 1, 'G': 64, 'OY': params['output_size'][2], 'OX': params['output_size'][3],
                                      'FY': params['kernel_size'], 'FX': params['kernel_size']},
    ...
    def _hyperparameterDA(self, N_GAMMA):
        ...
        for id in tqdm(range(N_GAMMA), ncols=80):
            uid = batch_space[id]
            config = self.searchspace.get_net_config(uid, 'cifar100') # cifar10, cifar100, ImageNet16-120
        ...
        COMBINED_GAMMA_K = 2.68690173088039e-12  # if 0, run HDA
        COMBINED_GAMMA_Q = 1.02460061284506e-11  # if 0, run HDA

        # return GAMMA_K, GAMMA_Q
        return COMBINED_GAMMA_K, COMBINED_GAMMA_Q
